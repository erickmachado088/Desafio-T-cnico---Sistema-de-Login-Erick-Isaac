...
usamos const para deixar as variaveis imutaveis ou melhor constantes.
'const variavel = valor adicionado'

o require é uma função interna do ambiente Node.js usada para importar módulos, ou seja, para carregar arquivos, bibliotecas ou pacotes externos em seu código
...
(obs: com if para contornar alguns problemas como informações faltando no cadastro, informações ja existentes, dados incorretos no login.)

app.listen(PORT, () => {
	console.log('Mensagem escolhida ${PORT}');
